...
// resolution of MIDI High resolution, actual value is 128 higher, values below
// 128 are note off
static const uint16_t MIDI_HIGHRES_RESOLUTION = 16128;
static const uint16_t MIDI_HIGHRES_OFFSET = 128;
...
static const float NOTEON_TIME_RANGE =
    (float)(NOTEON_TIME_MAX - NOTEON_TIME_MIN);

// Curve for the noteon velocity. Change only the value in the middle.
static const float NOTEON_VELOCITY_BIAS = pow(1 - 0.65f, 3);
...
// precalculate timing range
static const float NOTEOFF_TIME_RANGE =
    (float)(NOTEOFF_TIME_MAX - NOTEOFF_TIME_MIN);
...
static const uint8_t PEDAL_COUNT = 2;
...
inline uint16_t calculate_midi_noteon_velocity(uint32_t time) {
  float x = NOTEON_TIME_RANGE - (float)time / NOTEON_TIME_RANGE;

  float midi_velocity = x * NOTEON_VELOCITY_BIAS /
                            (x * NOTEON_VELOCITY_BIAS - x + 1) *
                            MIDI_HIGHRES_RESOLUTION +
                        MIDI_HIGHRES_OFFSET;

  return (uint16_t)midi_velocity;
}

inline uint8_t calculate_midi_noteoff_velocity(uint32_t time) {
  float midi_velocity = (NOTEOFF_TIME_RANGE - (float)time) / NOTEOFF_TIME_RANGE * 127);

  return (uint8_t)midi_velocity;
}
...
void setup() {
  ...
  pedals[0].mode = SINGLE;
  pedals[0].midi_cc0 = 64;

  pedals[1].mode = SINGLE;
  pedals[1].midi_cc0 = 65;
...
void loop() {
  ...
  for (int key = 0; key < KEY_COUNT; key++) {
    ...
    if (keys[key].debounce_0 < current_debounce) {
      if (keys[key].trigger_0 && !keys[key].prev_trigger_0 &&
        ...
        uint8_t velocity = calculate_midi_noteoff_velocity(key);

        usbMIDI.sendNoteOff(key + MIDI_KEY_OFFSET, velocity, MIDI_CHANNEL);
      }
    }
    ...
    if (keys[key].debounce_1 < current_debounce) {
      if (keys[key].trigger_1 && !keys[key].prev_trigger_1 &&
        ...
        uint16_t velocity = calculate_midi_noteon_velocity(keys[key].time);

        usbMIDI.sendControlChange(MIDI_HIGHRES_CC, (uint8_t)(velocity & 0x7f),
                                  MIDI_CHANNEL);
        usbMIDI.sendNoteOn(key + MIDI_KEY_OFFSET, (uint8_t)(velocity >> 7),
                           MIDI_CHANNEL);
  ...
  for (int pedal = 0; pedal < PEDAL_COUNT; pedal++) {
      ...
      if (pedals[pedal].debounce_1 < current_debounce) {
        if (digitalRead(pedals[pedal].pin_ring)) {
          ...
          if (!pedals[pedal].prev_p1) {
            usbMIDI.sendControlChange(MIDI_CHANNEL, pedals[pedal].midi_cc1,
      ...
      if (pedals[pedal].debounce_0 < current_debounce) {
        if (digitalRead(pedals[pedal].pin_tip)) {
          ...
          if (!pedals[pedal].prev_p0) {
            usbMIDI.sendControlChange(pedals[pedal].midi_cc0,
    ...
    case ANALOG: {
      pedals[pedal].analog_current_value = analogRead(pedals[pedal].pin_ring);
      ...
      if (pedals[pedal].analog_current_value !=
        ...
        usbMIDI.sendControlChange(
            ...
            map(pedals[pedal].analog_current_value,
                pedals[pedal].analog_min_value, pedals[pedal].analog_max_value,
                pedals[pedal].midi_min_value0, pedals[pedal].midi_max_value0),
            MIDI_CHANNEL);
        pedals[pedal].analog_last_value = pedals[pedal].analog_current_value;
      }
    }; break;
    }
  }
